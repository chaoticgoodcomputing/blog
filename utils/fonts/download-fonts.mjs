  ...
  for (const weight of weights) {
    for (const style of styles) {
      ...
      try {
        ...
        console.log(`  ↓ ${weight}${isItalic ? " italic" : ""}...`)
        ...
        if (!urlMatch) {
          console.warn(`  ⚠️  No woff2 URL found in CSS response`)
          continue
        }
...
@font-face {
  ...
  src: url('/static/fonts/${familySlug}/${filename}') format('woff2');
        ...
        console.error(`  ✗ Failed to download ${weight}${isItalic ? " italic" : ""}: ${error.message}`)
      }
    }
  }
  ...
  if (cssRules.length > 0) {
    ...
    console.log(`  ✓ Generated ${familySlug}.css\n`)
  } else {
    console.warn(`  ⚠️  No fonts downloaded for ${family}\n`)
  }
}

async function generateMasterCSS() {
  console.log("📝 Generating master fonts.css...")
...
async function main() {
  console.log("🔤 Font Download Utility\n")
