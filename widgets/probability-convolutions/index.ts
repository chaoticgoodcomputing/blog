import { ProbabilityConvolutions } from "./component.tsx"
import script from "./script.inline.ts"
import css from "./style.inline.scss"

// Import types from the global widget system
import type { WidgetDefinition } from "../../../../quartz/widgets/types"

export const probabilityConvolutions: WidgetDefinition = {
  Component: ProbabilityConvolutions,
  script: script,
  css: css,
  selector: ".widget-probability-convolutions",
  scriptName: "probability-convolutions",
}
